...
public class PastaManager : MonoBehaviour 
{
    ...
    /// </summary>
    private List<Pasta> _pastas = new List<Pasta>();
    ...
    {	
        ...
        foreach (Pasta pasta in _pastas)
        {
			if (pasta.State != Pasta.STATE.Dead)
			{
            	pasta.FrameUpdate();
			...
			if (pasta.State == Pasta.STATE.Dead)
			{
                switch (pasta.pastaType)
                {
                    case Pasta.TYPE.Pizza:
                        ...
                        if (_livePizzaCount < Mathf.Min(maxLivePizzas, gameManager.GetCurrentPastaTypeCount(Pasta.TYPE.Pizza)))
                        {
